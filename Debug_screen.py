...
servo_control_title = ""
servo_control_ids: List[int] = []
servo_control_idx = 0
servo_control_angles: Dict[int, int] = {}
SERVO_STEP_FAST = 5
SERVO_STEP_FINE = 1
...
class DebugContext:
    ...
    def get_led(self):
        if self.led is None:
            LED = self.import_class(["Robot_lightcontrol"], "LED")
            self.led = LED(self.get_robot())
        return self.led
    ...
    def _ensure_mic_stream(self, mic):
        """Ensure MicManager.stream is alive before ASR/VAD/WakeWord reads from it."""
        if mic is None:
            return None
        ...
        if stream is not None:
            try:
                stream_alive = bool(getattr(stream, "active", False))
            except Exception:
                stream_alive = True

        if stream is None or not stream_alive:
            try:
                mic.start_mic()
            except Exception:
                # Some sounddevice versions do not expose .active reliably;
                # retry by closing the old stream then starting again.
                try:
                    if hasattr(mic, "stop"):
                        mic.stop()
                except Exception:
    ...
    def get_mic(self):
        # Prefer the listener mic if listener already exists, so the same audio
        # stream is shared by MIC_LEVEL, VAD, ASR, and WakeWord tests.
        if self.listener is not None and hasattr(self.listener, "mic"):
            return self._ensure_mic_stream(self.listener.mic)
        ...
        return self._ensure_mic_stream(self.mic)

    def get_listener(self):
        if self.listener is None:
            # If a standalone MIC_LEVEL/VAD stream was opened before ASR/WakeWord,
            # close it first to avoid two InputStreams fighting for the same USB mic.
            ...
            # HumandroidListen.start() returns early when running=True, so if a
            # previous test closed the stream, force-start the underlying mic here.
            self._ensure_mic_stream(self.listener.mic)
    ...
    def close_all(self):
        for obj_name in ["hand", "speaker", "listener", "mic", "camera", "robot"]:
...
def execute_debug_command(cmd: str):
    ...
    # LED
    if cmd.startswith("LED_MODE_"):
        return led_mode(cmd.replace("LED_MODE_", ""))
    if cmd == "LED_OFF":
        return led_off()
    if cmd == "LED_CUSTOM":
        return led_custom_input()
...
SERVO_GROUPS = {
    ...
    "HEAD": [1, 2],
    "RIGHT ARM": [3, 4, 5, 6, 7],
    "RIGHT HAND": [8, 9, 10, 11, 12],
    "LEFT ARM": [19, 20, 21, 22, 23],
    "LEFT HAND": [24, 25, 26, 27, 28],
}
...
def load_servo_angles() -> Dict[int, int]:
    data = load_yaml_file(SERVO_FILE, default={}) or {}
    servos = data.get("servos", data) or {}
    angles = dict(SERVO_DEFAULTS)
...
def save_servo_angle(servo_id: int, angle: int):
    data = load_yaml_file(SERVO_FILE, default={}) or {}
    servos = data.get("servos", {}) or {}
    servos[int(servo_id)] = int(angle)
    data["servos"] = servos
    save_yaml_file(SERVO_FILE, data)


def send_servo_realtime(servo_id: int, angle: int):
    servo_id = int(servo_id)
    angle = int(clamp(int(angle), 0, 180))
    robot = ctx.get_robot()
    result = robot.move_servo(servo_id, angle)
    save_servo_angle(servo_id, angle)

    # keep head state synchronized when controlling head servos directly
    if servo_id == 1:
        global head_pitch
        head_pitch = angle
        if ctx.head is not None:
            ctx.head.current_pitch = angle
    elif servo_id == 2:
        global head_yaw
        head_yaw = angle
        if ctx.head is not None:
            ctx.head.current_yaw = angle

    return result


def draw_servo_control_screen():
    img, draw = new_canvas()
    title = servo_control_title or "SERVO CONTROL"

    bbox = draw.textbbox((0, 0), title, font=font_title)
    tw = bbox[2] - bbox[0]
    draw.text(((SCREEN_W - tw) // 2, 18), title, fill="white", font=font_title)

    y = 68
    for i, sid in enumerate(servo_control_ids):
        angle = servo_control_angles.get(sid, SERVO_DEFAULTS.get(sid, 90))
        selected = i == servo_control_idx
        fill = "yellow" if selected else "white"
        prefix = ">" if selected else " "
        draw.text((32, y), f"{prefix} ID {sid:02d}: {angle:3d} deg", fill=fill, font=font_text)
        y += 34
    ...
    for line in help_lines:
        draw.text((20, y), line, fill="cyan" if "SEND" in line else "white", font=font_small)
        y += 24

    show(img)


def enter_servo_control(title: str, servo_ids: List[int]):
    global screen_mode, servo_control_title, servo_control_ids, servo_control_idx, servo_control_angles

    screen_mode = "servo_control"
    servo_control_title = title
    servo_control_ids = [int(x) for x in servo_ids]
    servo_control_idx = 0
    servo_control_angles = load_servo_angles()

    log(f"SERVO CONTROL {title}: {servo_control_ids}")
    draw_servo_control_screen()


def move_servo_control(key_char: str):
    global servo_control_idx

    if not servo_control_ids:
        return

    c = key_char.lower()

    if c == "w":
        servo_control_idx = (servo_control_idx - 1) % len(servo_control_ids)
        draw_servo_control_screen()
        return
    ...
    sid = servo_control_ids[servo_control_idx]
    current = int(servo_control_angles.get(sid, SERVO_DEFAULTS.get(sid, 90)))

    if c == "a":
        current -= SERVO_STEP_FAST
    elif c == "d":
        current += SERVO_STEP_FAST
    elif c == "z":
        current -= SERVO_STEP_FINE
    elif c == "c":
        current += SERVO_STEP_FINE
    else:
        return

    current = int(clamp(current, 0, 180))
    servo_control_angles[sid] = current

    try:
        send_servo_realtime(sid, current)
        log(f"SERVO ID {sid} -> {current}")
    except Exception as e:
        log(f"SERVO ERROR ID {sid}: {e}")

    draw_servo_control_screen()


def resend_current_servo():
    if not servo_control_ids:
        return
    sid = servo_control_ids[servo_control_idx]
    angle = int(servo_control_angles.get(sid, SERVO_DEFAULTS.get(sid, 90)))
    try:
        send_servo_realtime(sid, angle)
        log(f"SERVO RESEND ID {sid} -> {angle}")
    except Exception as e:
        log(f"SERVO RESEND ERROR: {e}")
    draw_servo_control_screen()


def move_servo_input():
    global input_mode
    input_mode = True
    draw_text_screen("MOVE SERVO\nType: ID ANGLE", size=21, color="cyan")

    try:
        text = input("\n[MOVE SERVO] Nhập dạng ID GÓC, ví dụ 1 110: ").strip()
        parts = text.split()
        if len(parts) != 2:
            return make_result(False, "Format: ID ANGLE")
        sid = int(parts[0])
        angle = int(parts[1])
        send_servo_realtime(sid, angle)
        return make_result(True, "Servo moved", [f"ID: {sid}", f"Angle: {angle}"])
    except Exception as e:
        return make_result(False, str(e))
    finally:
        input_mode = False
...
def servo_test_menu():
    log("SERVO TEST MENU")
    show_menu("SERVO TEST", [
        ("MOVE SERVO", lambda: run_cmd("MOVE_SERVO")),
        ("MOUTH 0", lambda: enter_servo_control("MOUTH", SERVO_GROUPS["MOUTH"])),
        ("HEAD 1-2", lambda: enter_servo_control("HEAD", SERVO_GROUPS["HEAD"])),
        ("R ARM 3-7", lambda: enter_servo_control("RIGHT ARM", SERVO_GROUPS["RIGHT ARM"])),
        ("R HAND 8-12", lambda: enter_servo_control("RIGHT HAND", SERVO_GROUPS["RIGHT HAND"])),
        ("L ARM 19-23", lambda: enter_servo_control("LEFT ARM", SERVO_GROUPS["LEFT ARM"])),
        ("L HAND 24-28", lambda: enter_servo_control("LEFT HAND", SERVO_GROUPS["LEFT HAND"])),
        ("BACK", motion_menu),
    ])
...
def led_mode(mode: str):
    def _mode():
        led = ctx.get_led()
        result = led.set_mode(mode)
        return make_result(True, f"LED {mode}", [f"Mode: {mode}", f"ESP32: {result}"])

    return call_safe(f"LED {mode}", _mode)
...
def led_off():
    def _off():
        led = ctx.get_led()
        result = led.off()
        return make_result(True, "LED off", ["Mode: OFF", f"ESP32: {result}"])

    return call_safe("LED off", _off)


def led_custom_input():
    global input_mode
    input_mode = True
    draw_text_screen("CUSTOM LED\neffect r g b", size=21, color="cyan")

    try:
        text = input("\n[CUSTOM LED] Nhập: effect r g b, ví dụ wave 0 150 255: ").strip()
        parts = text.split()
        if len(parts) != 4:
            return make_result(False, "Format: effect r g b")
        effect = parts[0]
        r, g, b = map(int, parts[1:4])
        led = ctx.get_led()
        result = led.custom_effect(effect, r, g, b)
        return make_result(True, "Custom LED", [f"Effect: {effect}", f"RGB: {r},{g},{b}", f"ESP32: {result}"])
    except Exception as e:
        return make_result(False, str(e))
    finally:
        input_mode = False
...
def led_test_menu():
    log("LED TEST MENU")
    show_menu("LED TEST", [
        ("IDLE", lambda: run_cmd("LED_MODE_IDLE")),
        ("THINK", lambda: run_cmd("LED_MODE_THINK")),
        ("LISTEN", lambda: run_cmd("LED_MODE_LISTEN")),
        ("HAPPY", lambda: run_cmd("LED_MODE_HAPPY")),
        ("INFO", lambda: run_cmd("LED_MODE_INFO")),
        ("RIGHT", lambda: run_cmd("LED_MODE_RIGHT")),
        ("ERROR", lambda: run_cmd("LED_MODE_ERROR")),
        ("CUSTOM", lambda: run_cmd("LED_CUSTOM")),
        ("OFF", lambda: run_cmd("LED_OFF")),
        ("BACK", main_menu),
    ])
...
def on_press(key):
    ...
    try:
        ...
        # SERVO REAL-TIME CONTROL MODE
        if screen_mode == "servo_control":
            if c in ["w", "a", "s", "d", "z", "c"]:
                move_servo_control(c)
            elif c == "q":
                servo_test_menu()
            elif c == "e":
                run_cmd("STOP_ROBOT")
        ...
        if key == keyboard.Key.enter and screen_mode == "servo_control":
            resend_current_servo()
            return
